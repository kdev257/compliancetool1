...
from app_appeal.models import Appellate_Order
# Create your models here.

class Tribunal_Appeal_Draft(models.Model):
...
class Tribunal_Appeal_Final(models.Model):
    order = models.ForeignKey(Order, on_delete=models.CASCADE)
    app_order = models.ForeignKey(Appellate_Order, on_delete=models.CASCADE)
...
class Tribunal_Payment(models.Model):
    PAYMENT_CHOICES=(
...
class Tribunal_Hearing_Notice(models.Model):
    order = models.ForeignKey(Order,on_delete=models.CASCADE)
    app_order = models.ForeignKey(Appellate_Order,on_delete=models.CASCADE)
...
class Tribunal_Appeal_Hearing(models.Model):
    order = models.ForeignKey(Order,on_delete=models.CASCADE)
    app_order = models.ForeignKey(Appellate_Order,on_delete=models.CASCADE)
    hearing_date = models.DateField()
...
class Tribunal_Order(models.Model):
    OUTCOME_CHOICE=(
        ('Allowed','Allowed'),
        ('Rejected','Rejected'),
...
class Tribunal_Order_Details(models.Model):
    order = models.ForeignKey(Order,on_delete=models.CASCADE)
    app_order= models.ForeignKey(Appellate_Order,on_delete=models.CASCADE)
    issue = models.ForeignKey(Issue,on_delete=models.CASCADE)
    demand = models.PositiveIntegerField(default=0,verbose_name='Balance Demand as per Tribuanl Order')
