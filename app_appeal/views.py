...
def action(request,pk): #This View triggers master template for appeal proceedings\
    ...
    try:
        ...
        pi = None
    try:
        notice = Hearing_Notice.objects.get(order=pk)
    except:
        Hearing_Notice.DoesNotExist
        notice= None
    try:    
        hearing = App_Appeal_Hearing.objects.filter(order=pk).last().hearing_date
    except:
        App_Appeal_Hearing.DoesNotExist
        hearing= None
    try:
        order= Appellate_Order.objects.get(order=pk)
    except:
    ...
    cash = Payment.objects.filter(order=pk)           
...
def appeal_action(request,pk): 
    ...
    add = Addition.objects.filter(order=pk)     
...
def draft_appeal(request,pk):
        approved =list(Addition.objects.filter(order=pk).values_list('is_approved',flat=True))
...
def as_filed_appeal(request,pk):
    try:
        if Appellate_Appeal_Draft.objects.get(order=pk) is not None:       
            if request.method=='POST':
...
def create_hearing_notice(request,pk):
    try:
        if Appellate_Appeal_Final.objects.get(order=pk):
            if request.method=='POST':
...
def appeal_hearing(request,pk):
    pi = Hearing_Notice.objects.get(order=pk)
    if App_Appeal_Hearing.objects.last() is None:
        la= pi.hearing_date
    else:
        la = App_Appeal_Hearing.objects.filter(order=pk).last()
        la= la.next_date
    if request.method =='POST':
...
def appeal_order(request,pk):
    ...
    if App_Appeal_Hearing.objects.filter(order=pk).last().is_complete:
        if request.method =='POST':
...
def view_additions(request,pk):#View edits order_additions for updating appeal results
    pi = Addition.objects.filter(order=pk)
    return render(request,'app_appeal/view_additions.html',{'pi':pi,'pk':pk})    

@login_required
def appellate_order_details(request,pk,id):
    disp = tuple(Appellate_Order_Details.objects.filter(order=pk).values_list('demand',flat=True))
    # print(disp)
    t_demand = Appellate_Order.objects.get(order=pk).demand
    # print(t_demand)
    ...
    if request.method =='POST':
        ...
        if form.is_valid():
            try:
                ...
                instance.created_by= User.objects.get(id=request.user.id)
                instance.order = Order.objects.get(proceeding=pk)
                instance.app_order = Appellate_Order.objects.get(id=pk)
                instance.save()
...
def payment(request,pk):
    add= tuple(Addition.objects.filter(order=pk).values_list('disputed_additions',flat=True))
    adm= tuple(Addition.objects.filter(order=pk).values_list('admitted_additions',flat=True))
...
def view_appellate_order_details(request,pk):    
    notice = Appellate_Order_Details.objects.filter(order=pk)
